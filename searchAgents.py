...
def cornersHeuristic(state, problem):
    ...
    for x in corners:
        if x not in state[1]:
            totalcorners.append(x)
        
    current = state[0]
    cost = 0

    while len(totalcorners) > 0:
        distance, corner  = min([(abs( current[0] - corner[0] ) + abs( current[1] - corner[1] ), corner) for corner in totalcorners])
        cost += distance
        current = corner
        totalcorners.remove(corner)
